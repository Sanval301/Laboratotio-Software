...
const createFlight = async (req, res) => {
  ...
  if (isNaN(CostoPorPersona)) {
    return res
      .status(400)
      .json({ message: "El costo por persona debe ser un número válido" });
  }
...
const createCard = async (req, res) => {
  ...
  if (!/^[a-zA-Z\s]+$/.test(titular)) {
    return res
      .status(400)
      .json({
        error: "El nombre del titular solo debe contener letras y espacios",
      });
  }
  ...
  if (!/^(0[1-9]|1[0-2])\/[0-9]{2}$/.test(fechaExpiracion)) {
    return res
      .status(400)
      .json({ error: "La fecha de expiración debe estar en formato MM/YY" });
  }
  ...
  try {
    const card = flightService.createCard({
      numero,
      titular,
      fechaExpiracion,
      cvv,
...
const deleteCard = async (req, res) => {
  ...
  if (!numero) {
    return res
      .status(400)
      .json({ error: "El ID de la tarjeta es obligatorio" });
  }
...
const BuyTicket = async (req, res) => {
  const { nombre, email, vuelo, fechaVuelo, estado, tarjeta, fechacompra } =
    req.body;
  ...
  if (!/^[a-zA-Z\s]+$/.test(nombre)) {
    return res
      .status(400)
      .json({ error: "El nombre debe contener solo letras y espacios" });
  }
  ...
  try {
    ...
    if (ticketsCount >= 5) {
      return res
        .status(400)
        .json({ error: "No puedes comprar más de 5 tiquetes por persona" });
    }

    // Realizar compra
    const resultado = await flightService.BuyTicket(
      nombre,
      email,
      vuelo,
      fechaVuelo,
    ...
    res
      .status(200)
      .json({ mensaje: "Compra realizada exitosamente", resultado });
  } catch (error) {
...
const cancelBuy = async (req, res) => {
  ...
  try {
    ...
    if (BuyTicket.email !== email) {
      return res
        .status(403)
        .json({ error: "El correo electrónico no coincide con la compra" });
    }
    ...
    if (diferenciaHoras <= 24) {
      return res
        ...
        .json({
          error:
            "La cancelación solo es posible más de 24 horas antes del vuelo",
        });
    }
...
const reserveTicket = async (req, res) => {
  ...
  if (!/^\d{4}-\d{2}-\d{2}$/.test(fecha)) {
    return res
      .status(400)
      .json({
        error: "La fecha no es válida, debe tener el formato YYYY-MM-DD",
      });
  }
  ...
  try {
    ...
    if (totalTiquetes >= 5) {
      return res
        ...
        .json({
          error:
            "Solo puedes reservar o comprar un máximo de 5 tiquetes por vuelo",
        });
    }

    // Crear reserva
    const resultado = await flightService.crearReserva(
      nombre,
      email,
      vuelo,
      fecha
...
const cancelReservation = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ mensaje: "Reserva cancelada exitosamente", resultado });
  } catch (error) {
...
const createAdmin = async (req, res) => {
  ...
  try {
    ...
    const adminId = await adminService.crearAdministrador(
      nombre,
      apellido,
      email,
      nombreUsuario,
    ...
    res
      .status(201)
      .json({ mensaje: "Administrador creado exitosamente", adminId });
  } catch (error) {
